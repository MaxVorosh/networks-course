...
![alt text](images/wireshark_lease.png)
...
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   ...
   - 0x89a8e2e6. Оно необходимо, чтобы при броадкасте хосты понимали, что сервер отвечает именно им.
...
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   вашем эксперименте?
   - Срок аренды нужен, чтобы периодически сбрасывать IP-адреса устройствам, которые больше не подключены к сети. В моём случае, это время - 2 часа.
...
1. Сразу будем рассматривать $p \in (0, 1)$, т.к. при $p = 0$ или $p = 1$ эффективность равна 0.

   $f(p) = N \cdot p(1-p)^{N-1}$
   
   $f'(p) = N((1-p)^{N-1} - p(N-1)(1-p)^{N-2})$

   $f'(p) = 0$. $(1-p)^{N-1} - p(N-1)(1-p)^{N-2} = 0$
   
   $1 - p - p(N-1) = 0$
   
   $p = \frac{1}{N}$

   Посмотрим на знаки производной слева и справа от этой точки. Сразу можно вынести за скобки положительный множитель $N(1-p)^{N-2}$, и следить только за множителем $1 - p - p(N-1) = 1 - pN$.
   
   $1 - \frac{1}{2N} \cdot N > 0$
...
2. $\lim\limits_{N \rightarrow \infty} N\frac{1}{N}(1 - \frac{1}{N})^{N-1} = \lim\limits_{N \rightarrow \infty} (1 - \frac{1}{N})^{-1} \cdot (1 - \frac{1}{N})^N = 1 ^ {-1} \cdot e ^ {-1} = e ^ {-1}$
