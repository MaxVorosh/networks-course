...
#### Демонстрация работы
Картинка, которая есть на сервере
![alt text](images/server_good.png)
Картинка, которой нет
![alt text](images/server_bad.png)
...
#### Демонстрация работы
Попросим содержимое существующего файла
![alt text](images/client_good.png)
А теперь не существующего
![alt text](images/client_bad.png)
Теперь постучимся не в тот порт
![alt text](images/client_bad_server.png)
...
#### Решение
Сначала посчитаем задержку на хосте А. Чтобы собрать (и потом начать отправлять) пакет, ему нужно $\frac{56 \cdot 8}{128 \cdot 10^3} = 0.4375$ мс. Далее мы должны передать пакет целиком на хост Б. $d_{передачи} = \frac{L}{R} = \frac{56 \cdot 8}{10^6} = 0.448$ мс. Итого, задержка составит $5.8855$ мс.
...
#### Решение
Т.к. среднее число пакетов в буфере равно 10, то в формуле Литтла $N = 11$ (нужно посчитать пакет, готовящийся отправиться). $d_{передачи} = 0.01$ c. Значит, $d = d_{ожидания} + d_{передачи} = 0.02$ c. Отсюда, $\alpha = \frac{N}{d} = 550$ пакетов/с
...
#### Решение
1. Временная разница будет равна задержке второго пакета на первой линии связи. Задержка ожидания равна 0, т.к. пока второй пакет дойдёт до маршрутизатора, первый пакет успеет дойти до клиента. Значит, временная разница будет равна $\frac{L}{R_{S}} + d_{распространения}$
2. Да, может. Поскольку скорость передачи второй линии меньше, то второй пакет может прийти на маршрутизатор тогда, когда первый ещё передаётся по второй линии связи. $T = (d_{передачи2} + d_{распространения}) - (d_{передачи1} + d_{распространения}) = \frac{L}{R_{C}} - \frac{L}{R_{S}}$
...
#### Решение
1. $\Delta = \frac{L}{R} = \frac{85\cdot 10^4}{15 \cdot 10^6} = \frac{17}{300}$ с
2. $d_{ответа} = 3 + \frac{\frac{17}{300}}{1 - 16 \cdot \frac{17}{300}} = 3 + \frac{17}{28} \approx 3.61$ с
3. $d_{ответа} = 3 + \frac{\frac{17}{300}}{1 - 16 \cdot 0.4 \cdot \frac{17}{300}} = 3 + \frac{17}{191.2} \approx 3.09$ с
