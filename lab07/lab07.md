...
2. Каков IP-адрес у сервера gaia.cs.umass.edu? Каковы номера портов для отправки и приема
   ...
   - 128.119.245.12
   - 80 и для отправки, и для приёма
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - 0
   - 1
   - Добавил единицу к порядковому номеру пришедшего пакета
   - Среди флагов есть SYN и ACK
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - 100 КБ/с
   - За время RTT мы передали 10 пакетов по 1460 байт. Значит, за единицу времени мы передаём $\frac{14600}{0.142361} = 102556 Б/с = 100 КБ/с$
...
![alt text](images/wiresharkStevens.png)
...
![alt text](images/udp2_1.png)

![alt text](images/udp2_2.png)

![alt text](images/udp2_3.png)
