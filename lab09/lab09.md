...
![alt text](images/wireshark_request.png)

![alt text](images/wireshark_response.png)
...
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - Это протокол транспортного уровня, он не работает с портами. Это задача протоколов транспортного уровня
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   ...
   - Контрольная сумма, идентификатор, порядковый номер
   - По 2 байта, всего 6.
...
![alt text](images/tracert_error_response.png)

![alt text](images/tracert_noerror_response.png)

![alt text](images/tracert_logs.png)
...
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - Часть полей отмечена unused. Также есть поле с длиной исходного сообщения. И перед данными идут 8 байт заголовка исходного сообщения.
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Эти пакеты отличаются типом, и отстутствием полей из прошлого пункта.
   - Эти сообщения дошли до адресата, поэтому не содержат в себе ошибку.
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Таких канала 2: на 13 и на 15 шаге задержка существенно вырастает.
   - Первый канал -- из Копенгагена в Нью-Йорк, второй -- из Нью-Йорка в Канзас
